    ...
    <div className="mx-auto max-w-7xl animate-fade-in px-4 py-8 sm:px-6">
      ...
      {loading && (
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {[1, 2, 3, 4, 5, 6, 7, 8].map((n) => (
            <div
              key={n}
              className="flex flex-col gap-3 rounded-xl border border-border p-3 shadow-sm"
            >
      ...
      {!loading && !error && hotels.length === 0 && (
        <div className="rounded-xl border border-dashed border-border p-12 text-center text-muted-foreground">
          No hotels found for your search criteria. Please try adjusting your filters or
          destination.
        </div>
      )}
